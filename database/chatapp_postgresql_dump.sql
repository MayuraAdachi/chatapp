...
-- メッセージ数制限のトリガー関数（300件制限）
CREATE OR REPLACE FUNCTION limit_room_messages()
RETURNS TRIGGER AS $$
DECLARE
...
BEGIN
    -- ルームの最大メッセージ数を取得
    SELECT max_message_count INTO max_count
    ...
    -- 制限を超えた場合、古いメッセージを削除
    IF message_count >= max_count THEN
        ...
        AND id IN (
            ...
            LIMIT (message_count - max_count + 1)
        );
    END IF;
...
-- メッセージ数制限トリガー
CREATE TRIGGER trigger_limit_messages
    AFTER INSERT ON messages
    FOR EACH ROW
    EXECUTE FUNCTION limit_room_messages();
...
('admin', 'admin@example.com', crypt('admin123', gen_salt('bf')), '管理者', 'admin'),
('developer', 'dev@example.com', crypt('dev123', gen_salt('bf')), '開発者', 'developer'),
('user1', 'user1@example.com', crypt('user123', gen_salt('bf')), 'ユーザー1', 'member');

-- サンプルセッション（匿名ユーザー）
INSERT INTO sessions (id, display_name, expires_at) VALUES
('session_001', '匿名ユーザー1', CURRENT_TIMESTAMP + INTERVAL '1 day'),
('session_002', '匿名ユーザー2', CURRENT_TIMESTAMP + INTERVAL '1 day');

-- サンプルルーム作成
INSERT INTO rooms (name, description, type, created_by_user_id) VALUES
('一般チャット', '誰でも参加できる一般的なチャットルームです', 'group', 1),
('開発者向け', '開発に関する話題専用', 'group', 2);

-- 鍵付きルーム
INSERT INTO rooms (name, description, type, is_private, password_hash, created_by_user_id) VALUES
('プライベートルーム', 'パスワード保護されたルーム', 'group', TRUE, crypt('secret', gen_salt('bf')), 1);
...
-- 匿名ユーザーをルームに追加
INSERT INTO room_members (room_id, session_id, display_name, role) VALUES
(1, 'session_001', '匿名ユーザー1', 'member'),
(1, 'session_002', '匿名ユーザー2', 'member');

-- サンプルメッセージ
INSERT INTO messages (room_id, user_id, display_name, content) VALUES
(1, 1, '管理者', 'チャットルームへようこそ！'),
(1, 2, '開発者', 'よろしくお願いします'),
(1, 3, 'ユーザー1', 'こんにちは！');

-- 匿名ユーザーのメッセージ
INSERT INTO messages (room_id, session_id, display_name, content) VALUES
(1, 'session_001', '匿名ユーザー1', '匿名でも参加できるんですね'),
(1, 'session_002', '匿名ユーザー2', 'はじめまして');

-- システムメッセージ
INSERT INTO messages (room_id, user_id, display_name, content, message_type) VALUES
(1, 1, 'システム', 'ユーザー1が参加しました', 'system');
...
BEGIN
    RAISE NOTICE '==============================================';
    RAISE NOTICE 'チャットアプリ データベース作成完了！';
    RAISE NOTICE '==============================================';
    RAISE NOTICE 'サンプルデータ:';
    RAISE NOTICE '- 管理者: admin / admin123';
    RAISE NOTICE '- 開発者: developer / dev123';
    RAISE NOTICE '- 一般ユーザー: user1 / user123';
    RAISE NOTICE '- 鍵付きルーム パスワード: secret';
    RAISE NOTICE '==============================================';
END
$$;
